...
int  ObjectGrabber::grab_object(int object_id,geometry_msgs::PoseStamped object_pose_stamped){
    ...
    ROS_INFO("planning motion of gripper to knock off pose at:");
...
//grab-object function w/ 2 args is default strategy for approach, grasp depart:
int  ObjectGrabber::knock_right(int object_id,geometry_msgs::PoseStamped object_pose_stamped){
    //given gripper_ID, object_ID and object poseStamped,
    // and assuming default approach, grasp and depart strategies for this object/gripper combo,
    // compute the corresponding required gripper-frame poses w/rt a named frame_id
    // (which will be same frame_id as specified in object poseStamped)
    int rtn_val;
    ROS_WARN("In object grabber trying to knock right ");
    bool success;
    if(!get_default_grab_poses(object_id,object_pose_stamped)) {
        ROS_WARN("no valid grasp strategy; giving up");
        return object_grabber::object_grabberResult::NO_KNOWN_GRASP_OPTIONS_THIS_GRIPPER_AND_OBJECT;
    ...
    ROS_WARN("preparing to try and knock this block off");
    //adjust the grasp post to move this thing off the table! 

    grasp_pose_.pose.position.y= grasp_pose_.pose.position.y-0.25; 
    xformUtils.printPose(grasp_pose_);
    rtn_val=arm_motion_commander_.plan_path_current_to_goal_gripper_pose(grasp_pose_);
    if (rtn_val != cartesian_planner::cart_moveResult::SUCCESS) return rtn_val; //return error code
    ROS_WARN("performing block knock off motion");
    rtn_val=arm_motion_commander_.execute_planned_path();
	ROS_WARN("block knock off motion complete");	
    
    return rtn_val; 
}

//grab-object function w/ 2 args is default strategy for approach, grasp depart:
int  ObjectGrabber::knock_left(int object_id,geometry_msgs::PoseStamped object_pose_stamped){
    //given gripper_ID, object_ID and object poseStamped,
    // and assuming default approach, grasp and depart strategies for this object/gripper combo,
    // compute the corresponding required gripper-frame poses w/rt a named frame_id
    // (which will be same frame_id as specified in object poseStamped)
    ...
    ROS_WARN("We're trying to knock the block left");
    //let's adjust the depart_post location 
    grasp_pose_.pose.position.y= grasp_pose_.pose.position.y+0.25; 
     
    xformUtils.printPose(grasp_pose_);
    rtn_val=arm_motion_commander_.plan_path_current_to_goal_gripper_pose(grasp_pose_);
    if (rtn_val != cartesian_planner::cart_moveResult::SUCCESS) return rtn_val; //return error code
    ROS_WARN("We just tried to knock the block left");
    rtn_val=arm_motion_commander_.execute_planned_path();
...
//grab-object function w/ 2 args is default strategy for approach, grasp depart:
int  ObjectGrabber::straddle_block(int object_id,geometry_msgs::PoseStamped object_pose_stamped){
    //given gripper_ID, object_ID and object poseStamped,
    // and assuming default approach, grasp and depart strategies for this object/gripper combo,
    // compute the corresponding required gripper-frame poses w/rt a named frame_id
    // (which will be same frame_id as specified in object poseStamped)
...
void ObjectGrabber::executeCB(const actionlib::SimpleActionServer<object_grabber::object_grabberAction>::GoalConstPtr& goal) {
    ...
    switch (action_code) {
        ...
        case object_grabber::object_grabberGoal::GRAB_OBJECT:
            ...
            object_grabber_as_.setSucceeded(grab_result_);
            break;
		case object_grabber::object_grabberGoal::KNOCK_OFF_TABLE_LEFT:
            ROS_INFO("GRAB_OBJECT: AND KNOCK IT LEFT!! ");
            object_id = goal->object_id;
            grasp_option = goal->grasp_option;
            object_pose_stamped_ = goal->object_frame;
            //get grasp-plan details for this case:
            ...
            rtn_val = knock_left(object_id,object_pose_stamped_);
            ROS_INFO("grasp attempt concluded");
            grab_result_.return_code = rtn_val;
            object_grabber_as_.setSucceeded(grab_result_);
            break;
		case object_grabber::object_grabberGoal::KNOCK_OFF_TABLE_RIGHT:
            ROS_INFO("GRAB_OBJECT: AND KNOCK IT RIGHT!! ");
            object_id = goal->object_id;
            grasp_option = goal->grasp_option;
            object_pose_stamped_ = goal->object_frame;
            //get grasp-plan details for this case:
            ...
            rtn_val = knock_right(object_id,object_pose_stamped_);
            ROS_INFO("grasp attempt concluded");
            grab_result_.return_code = rtn_val;
            object_grabber_as_.setSucceeded(grab_result_);
            break;
		case object_grabber::object_grabberGoal::STRADDLE_BLOCK:
            ROS_INFO("STRADDLE THE BLOCK!!!");
            object_id = goal->object_id;
            grasp_option = goal->grasp_option;
            object_pose_stamped_ = goal->object_frame;
            //get grasp-plan details for this case:
            ...
            rtn_val = straddle_block(object_id,object_pose_stamped_);
            ROS_INFO("we tried to straddle the block");
            grab_result_.return_code = rtn_val;
            object_grabber_as_.setSucceeded(grab_result_);
            break;
