...
int main(int argc, char** argv) {
	...
	while(ros::ok())
	{
		...
		if (g_result.object_type==0)
		{
			//send command to knock block left 
			ROS_INFO("sending a goal: knock the block left!!");
			g_goal_done = false;
			goal.action_code = coordinator::ManipTaskGoal::KNOCK_OFF_TABLE_LEFT;
		...
		else if (g_result.object_type==1)
		{
			//send command to knock block right 
			ROS_INFO("sending a goal: knock the block right!!");
			g_goal_done = false;
			goal.action_code = coordinator::ManipTaskGoal::KNOCK_OFF_TABLE_RIGHT;
			goal.pickup_frame = g_result.object_pose;
			goal.object_code= ObjectIdCodes::TOY_BLOCK_ID;
			action_client.sendGoal(goal, &doneCb, &activeCb, &feedbackCb);
			while (!g_goal_done) {
